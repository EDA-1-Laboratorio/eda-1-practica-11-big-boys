...
def particiona(arr: list, lo: int, hi: int) -> int:
    ...
    for j in range(lo, hi):
        # Si arr[j] es menor o igual al pivote, expandimos la región de menores
        if arr[j] <= pivot:
            i += 1
...
def particiona_aleatoria(arr: list, lo: int, hi: int) -> int:
    ...
    """
    # Elige un índice aleatorio entre lo y hi (inclusive)
    idx = random.randint(lo, hi)
    
    # Intercambia el elemento aleatorio con el último (que será el pivote)
    arr[idx], arr[hi] = arr[hi], arr[idx]
    
    # Llama a la partición estándar y devuelve su resultado
...
def quicksort(arr: list, lo: int = 0, hi: int = None) -> None:
    ...
    # PASO BASE
    # Si lo >= hi, el subarreglo tiene 0 o 1 elemento → ya está ordenado
    if lo >= hi:
        return
    ...
    # PASO RECURSIVO
    # 1. Particionamos el arreglo y obtenemos la posición del pivote
    p = particiona(arr, lo, hi)
    
    # 2. Llamada recursiva para la mitad izquierda
    quicksort(arr, lo, p - 1)
    
    # 3. Llamada recursiva para la mitad derecha
...
def quicksort_aleatorio(arr: list, lo: int = 0, hi: int = None) -> None:
    ...
    # Mismo esquema que quicksort, pero llamando a particiona_aleatoria
    # PASO BASE
    if lo >= hi:
        return
        
    # HIPÓTESIS INDUCTIVA (misma que arriba)
